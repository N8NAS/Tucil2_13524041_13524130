...
struct AABB {
    ...
    Vector3 getHalfExtents() const { 
        return {(max.x - min.x) / 2.0f, (max.y - min.y) / 2.0f, (max.z - min.z) / 2.0f}; 
    }
};
...
inline bool testAxis(const Vector3& axis, const Vector3& v0, const Vector3& v1, const Vector3& v2, const Vector3& extents) {
    ...
    float minP = min({p0, p1, p2});
    float maxP = max({p0, p1, p2});
...
inline bool checkIntersect(const AABB& box, const Triangle& tri) {
    ...
    if (max({v0.x, v1.x, v2.x}) < -e.x || min({v0.x, v1.x, v2.x}) > e.x) return false;
    if (max({v0.y, v1.y, v2.y}) < -e.y || min({v0.y, v1.y, v2.y}) > e.y) return false;
    if (max({v0.z, v1.z, v2.z}) < -e.z || min({v0.z, v1.z, v2.z}) > e.z) return false;
